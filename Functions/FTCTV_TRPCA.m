...
function [X, E, obj, err, iter] = FTCTV_TRPCA(M, opts)
...
while iter<max_iter
    ...
    for i = 1:n
        ...
        switch transform
            case 'DFT'
                [G{index},tnn_G{index},~,W{index}] = prox_htnn_F_Weight_Fair(porder_diff(X,index)+Gamma{index}/mu,1/(n*mu),W{index},tau(index)); 
            case 'DCT'
...
% 	E = prox_l1(M-X+Lambda/mu, lambda/mu);
    [E,W_L]=prox_l1_weight_Fair(M-X+Lambda/mu, lambda/mu,W_L,tauL);
